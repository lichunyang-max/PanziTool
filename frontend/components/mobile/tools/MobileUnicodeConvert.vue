...
/**
 * MobileUnicodeConvert.vue - 移动端 Unicode 转中文工具
 *
 * 单列布局：输入框在上，输出框在下
 * 操作按钮：解码/编码/清空 + 复制
 */
import { decodeUnicode, encodeUnicode } from '~/utils/tools/unicode'
...
function handleDecode() {
  ...
  const result = decodeUnicode(input.value)
...
function handleEncode() {
  ...
  const result = encodeUnicode(input.value)
  if (result.success && result.output !== undefined) {
    output.value = result.output
  } else {
  ...
  <div class="m-tool">
    ...
    <div class="m-tool__card">
      ...
      <textarea
        ...
        placeholder="输入含 \\uXXXX 转义的文本（解码）或普通文本（编码）..."
    ...
    <div class="m-tool__actions">
      <button type="button" class="m-btn m-btn--primary" @click="handleDecode">解码</button>
      <button type="button" class="m-btn m-btn--secondary" @click="handleEncode">编码</button>
