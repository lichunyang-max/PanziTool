...
/**
 * MobileYamlFormatter.vue - 移动端 YAML 格式化/校验/转JSON 工具
 *
 * 逻辑忠实移植自 panziui/tools/yaml-formatter.html 的内联脚本（js-yaml）。
 * 单列布局：输入框在上，输出框在下。
 * 操作按钮：格式化/校验/转JSON/清空 + 复制 + 缩进选择。
 */
...
<template>
  <div class="m-tool">
    ...
    <div class="m-tool__actions">
      ...
      <button type="button" class="m-btn m-btn--secondary" @click="handleToJson">转JSON</button>
    ...
    <div v-if="errorMsg" class="m-tool__alert m-tool__alert--error" role="alert">
      <span class="m-tool__alert-title">YAML 语法错误</span>
    ...
    <div
      ...
      role="status"
    >
      <span class="m-tool__alert-title" style="color: #10b981">YAML 校验通过</span>
      <span class="m-tool__alert-desc">输入是合法的 YAML 格式。</span>
    </div>
  </div>
</template>
