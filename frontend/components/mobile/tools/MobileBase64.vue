...
/**
 * MobileBase64.vue - 移动端 Base64 编码/解码工具
 *
 * 单列布局：输入框在上，输出框在下
 * 操作按钮：编码/解码/清空 + 复制
 */
import { encodeBase64, decodeBase64, fileToBase64 } from '~/utils/tools/base64'
...
const output = ref('')
const errorMsg = ref('')
const charset = ref<'utf8' | 'ascii'>('utf8')
...
function handleEncode() {
  ...
  const result = encodeBase64(input.value, charset.value)
...
function handleDecode() {
  ...
  const result = decodeBase64(input.value, charset.value)
  if (result.success && result.output !== undefined) {
    output.value = result.output
  } else {
...
async function handleFilePick(event: Event) {
  ...
  try {
    ...
    errorMsg.value = `文件读取失败：${e instanceof Error ? e.message : '未知错误'}`
  }
}
...
<template>
  <div class="m-tool">
    ...
    <div class="m-tool__card">
      ...
      >
        <span>{{ fileName ? `已选: ${fileName}` : '选择文件' }}</span>
      </button>
    </div>
    ...
    <div class="m-tool__card">
      <div class="m-tool__row">
        <span class="m-tool__label">字符编码</span>
        <select v-model="charset" class="m-tool__select">
          <option value="utf8">UTF-8 (推荐)</option>
