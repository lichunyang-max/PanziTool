...
/**
 * pages/mobile/image-tools/[slug].vue - 移动端图片工具详情页
 *
 * 类似开发者工具详情页，广告位使用 img_tool_middle
 * - 使用 MobileToolLayout 组件包裹
 * - 动态加载对应工具交互组件
 * - 中间广告位使用 MobileAdCard（ad_location_symbol='img_tool_middle'）
 * - 底部使用说明区（FAQ）
 *
...
const categoryRoute = computed(() => {
  if (toolMeta.value.category === 'developer') {
    return '/mobile/tools'
  }
...
const seoDescription = computed(() => toolMeta.value.description || staticMeta.value?.description || `${seoTitle.value} - 盘子工具站在线图片工具`)
  ...
  <div class="mobile-tool-page">
    ...
    <template v-else>
      <div class="mobile-placeholder">
        ...
        <NuxtLink to="/mobile/image-tools" class="mobile-placeholder__back">
          返回图片工具列表
        </NuxtLink>
      </div>
    </template>
