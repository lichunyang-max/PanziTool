 ...
 *
 * <p>核心防刷能力：通过 {@link #existsByToolIdAndAnonId} 检查 (tool_id, anon_id) 是否已存在，
 * 配合数据库唯一约束 {@code uk_tool_likes_tool_anon} 确保同一匿名用户对同一工具仅能点赞一次。</p>
 */
@Repository
public interface ToolLikeRepository extends JpaRepository<ToolLike, Long> {
     ...
     * @return 已存在返回 true
     */
    boolean existsByToolIdAndAnonId(Long toolId, String anonId);
